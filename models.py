    ...
    if config.model.class_name == "BertClassifier":
        ...
        try:
            if config.model.pt is None:
                raise RuntimeError("`config.pt` is null")
            model = ModelClass.from_pretrained(config.model.pt)
            pt_loaded = True
            LOG.info(f"Loaded model with hf's from_pretrained fron {config.model.pt}")
        except Exception:
            model = ModelClass.from_pretrained(config.model.name, cache_dir=scr())

    if not pt_loaded and config.model.pt is not None:
