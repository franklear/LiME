...
def report_tensors_in_mem():
    by_type_shape = {}
    for obj in gc.get_objects():
        try:
            if torch.is_tensor(obj) or (hasattr(obj, 'data') and torch.is_tensor(obj.data)):
                k = (type(obj), obj.shape)
                if k not in by_type_shape:
                    by_type_shape[k] = 0
                by_type_shape[k] += 1
        except:
            pass
    rep = 'tensors in memory (from gc):'
    for k, v in sorted(by_type_shape.items(), key=lambda t: (-functools.reduce(operator.mul, t[0][1], 1) * t[1], t[1], str(t[0][0]))):
        rep += f'\n    {k[0]}: {k[1]} * {v}'
    LOG.debug(rep)
    return by_type_shape, rep


def chech_graphic_mem(log_prefix: str = ''):
    LOG.debug(f"{log_prefix} {torch.cuda.memory_reserved()=} {torch.cuda.memory_allocated()=}")
...
def shift_targets(config):
    return "bert" not in config.model.name.lower() \
           and "t5" not in config.model.name.lower()


def scr():
    # using default caching dir
    return None
...
class RunningStatAverager:
    def __init__(self, suffix="", exclude: typing.Optional[typing.List] = None, compute_ppl: bool = True):
...
class EditBatchSampler:
    def __init__(self, n, n_edits=1, memorize_mode=False, loc_disjoint=True, seed=0, indices=None, loc_n=None):
        ...
        self.loc_n = loc_n if loc_n is not None else n
        self._init(self.indices)

    def _init(self, indices):
        self.perm = self.rng.permutation(self.n)
        if indices is not None:
            assert len(indices) == self.n
            self.perm = indices[self.perm]
    ...
    def sample(self, batch_size):
        ...
        loc_idxs = self.rng.choice(self.loc_n, batch_size - self.n_edits)
