...
class FT(EditableModel):
    ...
    def edit(self, batch, condition=None, language_idx=None, detach_history=False, update_params=False, virtual_batch_size=None):
        assert not update_params, "algorithm `ft` has no param to update"
        assert language_idx is None, "algorithm `ft` needs no language idx"
        assert virtual_batch_size is None, "algorithm `ft` doesn't support virtual batch"
        ...
        residuals = {k: v[0] @ v[1] if len(v) == 2 else v[0] for k, v in packed_residuals.items()}
        return FT(edit_model, self.config, self.model_constructor, self.edit_loss_fn), residuals, {}
